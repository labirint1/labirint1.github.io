...
$(function() {
   ...
   */
    ...
    function copied(status){
        ...
        if (status == "+") {
            ...
            setTimeout(function() {$(".setLabl").removeClass("setX2");}, 0);
            hist();
            copiedOFF();
            $("#key").addClass("newKey");
            newKey();
  ...
  $('#formSwitch').change(function() {
      if(getCookie("form")===undefined){sendCookie("form","top"); }
      state = getCookie("form");
      if(getCookie("form")=="top"){sendCookie("form","bottom");Ya('FUNC',"FORMBOTTOM");
      }else if(getCookie("form")=="bottom"){sendCookie("form","top");Ya('FUNC',"FORMTOP");}
      fotmPosition();
  });
...
$("#fPosition").prop("checked");
   ...
   */
    ...
    function hist(cap){
        if (history[0] != key) {
            ...
            if(cap !== true){history.unshift(key);}
            history.length=5;
  ...
  */
    ...
    function Ya(type,param,time){
        if(typeof yaCounter25355216!=="undefined"){
            ...
            if(type=='visit'){
                ...
                yaCounter25355216.reachGoal('VISIT', {"VISIT":visit});
            }
            if(type=='click'){
                ya1++;
                ...
                clickEdAll[use[1]] = 1;
                sendCookie("use",use);console.log(use);
                yaCounter25355216.reachGoal('CLICK', {"CLICK":key, USE:{CLICKALL:use[0],CLICK:ya1,CLICKED:clickEd,CLICKEDALL:clickEdAll}});
            }
            if(type=='copy') {
                ya2++;
                ...
                copyEdAll[use[2]] = 1;
                sendCookie("use",use);console.log(use);
                yaCounter25355216.reachGoal('COPY',  {"COPY": key, USE:{COPYALL: use[1],COPY: ya2,COPYED :copyEd, COPYEDALL :copyEdAll }});
            }
            if(type=='FUNC') {
                yaCounter25355216.reachGoal('FUNC',  {"FUNC": param});
    ...
    $(".winYa").click(function() {Ya('FUNC',"OPENWIN");});
    $(".help").click(function() {Ya('FUNC',"SOS");});
    $(".shareYa").click(function() {Ya('FUNC',"SHARE");});
...
$(function() {
  ...
  if($("#oneVk")) {
    ...
    *Tutorial
    */
 $('.help').click(function(){startIntro();});
    function startIntro(){
        var intro = introJs();
            intro.setOptions({
                steps: [
          ...
          intro.oncomplete(function(){console.log("end of introduction");}).onexit(function() {
        console.log("STOP");
